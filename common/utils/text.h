...
namespace NATBuster::Utils {
    ...
    inline std::string convert_os2app(const string_os& data) {
        ...
        WideCharToMultiByte(CP_UTF8, 0, data.data(), (int)data.size(), &res[0], size_needed, NULL, NULL);
        return res;
    }
    ...
    inline string_os convert_app2os(const std::string& data) {
        ...
        MultiByteToWideChar(CP_UTF8, 0, data.data(), (int)data.size(), &res[0], size_needed);
        return res;
    }
#elif __linux__
