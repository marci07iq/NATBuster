...
namespace NATBuster::Crypto {
    ...
    PuKey::PuKey(PuKey&& other) noexcept : PKey(std::move(other)) {

    }
    PuKey& PuKey::operator=(PuKey&& other) noexcept {
        PKey::operator=(std::move(other));
        return *this;
    }

    bool PuKey::load_file_public(std::string filename) {
        //Dont overwrite
    ...
    bool PuKey::load_public(const Utils::ConstBlobView& in) {
        //Dont overwrite
        if (_key != nullptr) return false;
    ...
    bool PuKey::export_file_public(std::string filename) const {
        //Need to have a key
        if (_key == nullptr) return false;
    ...
    bool PuKey::export_public(Utils::BlobView& out) const {
        //Need to have a key
        if (_key == nullptr) return false;
    ...
    bool PuKey::copy_public_from(const PuKey& src) {
        Utils::Blob content;
        if (!src.export_public(content)) return false;
        if (!load_public(content)) return false;
        return true;
    }
    PuKey PuKey::copy_public() const {
        PuKey res;
        if (!res.copy_public_from(*this)) res.erase();
        return res;
    }

    bool PuKey::verify(const Utils::ConstBlobView& data_in, const Utils::ConstBlobView& sig_in) const {
    ...
    bool PuKey::fingerprint(Utils::BlobView& out) const {
        ...
        Utils::Blob hash_data = Utils::Blob::factory_empty((uint32_t)len);

        len = hash_data.size();
        EVP_PKEY_get_raw_public_key(_key, hash_data.getw(), &len);
        assert(len <= hash_data.size());

        hash_data.resize((uint32_t)len);
    ...
    PrKey::PrKey(PrKey&& other) noexcept : PuKey(std::move(other)) {

    }
    PrKey& PrKey::operator=(PrKey&& other) noexcept {
        PuKey::operator=(std::move(other));
        return *this;
    }

    bool PrKey::load_file_private(std::string filename) {
        //Dont overwrite
        if (_key != nullptr) return false;
    ...
    bool PrKey::load_private(const Utils::ConstBlobView& in) {
        //Dont overwrite
        if (_key != nullptr) return false;
    ...
    bool PrKey::export_file_private(std::string filename) const {
        //Need to have a key
        if (_key == nullptr) return false;
    ...
    bool PrKey::export_private(Utils::BlobView& out) const {
        //Need to have a key
        if (_key == nullptr) return false;
    ...
    bool PrKey::copy_private_from(const PrKey& src) {
        Utils::Blob content;
        if (!src.export_private(content)) return false;
        if (!load_private(content)) return false;
        return true;
    }
    PrKey PrKey::copy_private() const {
        PrKey res;
        if (!res.copy_private_from(*this)) res.erase();
        return res;
    }

    bool PrKey::generate(PKeyAlgo nid) {
        //Override protection
        if (_key != nullptr) return false;
    ...
    bool PrKey::generate_ec25519() {
        return  generate(PKeyAlgo::Ec25519);
    }
    bool PrKey::generate_ed25519() {
        return generate(PKeyAlgo::Ed25519);
    }

    bool PrKey::sign(const Utils::ConstBlobView& data_in, Utils::BlobView& sig_out) const {
        EVP_MD_CTX* md_ctx = EVP_MD_CTX_new();
        if (md_ctx == nullptr) return false;
    ...
    bool PrKey::ecdhe(PuKey& key_other, Utils::BlobView& secret_out) const {
        EVP_PKEY_CTX* ctx;
